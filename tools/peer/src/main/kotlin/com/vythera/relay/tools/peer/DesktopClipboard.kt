    ...
    private fun onEvent(event: NodeEvent) {
        when (event) {
            ...
            is NodeEvent.TransferFinished -> {
                ...
                if (transfer.kind != TransferKind.CLIPBOARD || transfer.direction != TransferDirection.INCOMING || transfer.status != TransferStatus.Completed) return
                val files = transfer.storedFiles.map { File(it.location) }
